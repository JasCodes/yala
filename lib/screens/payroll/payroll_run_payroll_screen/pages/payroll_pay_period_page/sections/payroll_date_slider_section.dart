...
class PayrollDateSliderSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Observer(builder: (context) {
        var selectedDayIndex = store.selectedDayIndex;
        return ListWheelScrollViewX(
          controller: controller,
          onSelectedItemChanged: (value) {
            store.selectedDayIndex = value;
          },
          scrollDirection: Axis.horizontal,
          itemExtent: 90 + 11.3,
          builder: (context, index) {
            var active = selectedDayIndex == index;
            var _border = null;
            if (active) {
              _border = Border.all(
            ...
            var dateTime = store.dayIndexToDateTime(index);

            var weekday = Intl().date('EEEE').format(dateTime);
            var day = Intl().date('dd').format(dateTime);
            var month = Intl().date('MMM yyyy').format(dateTime);
